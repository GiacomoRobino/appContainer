export class Particle{
    ...
    constructor(w:number,h:number, options: any, ctx: any){
        ...
        this.w = w;
         ...
         this.vector = {
             x : (Math.cos(Math.floor(Math.random() * 360)) * options.defaultSpeed + Math.random() * options.variantSpeed),

             y : (Math.sin(Math.floor(Math.random() * 360)) * options.defaultSpeed + Math.random() * options.variantSpeed)
            };
    }
